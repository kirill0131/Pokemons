...
class Sprites: Codable {
    ...
    var allSprites: [String] {
        ...
        if let backDefault = backDefault { allSprites.append(backDefault) }
        if let backFemale = backFemale { allSprites.append(backFemale) }
        if let backShiny = backShiny { allSprites.append(backShiny) }
        if let backShinyFemale = backShinyFemale { allSprites.append(backShinyFemale) }
        if let frontDefault = frontDefault { allSprites.append(frontDefault) }
        if let frontFemale = frontFemale { allSprites.append(frontFemale) }
        if let frontShiny = frontShiny { allSprites.append(frontShiny) }
        if let frontShinyFemale = frontShinyFemale { allSprites.append(frontShinyFemale) }
